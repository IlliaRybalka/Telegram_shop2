...
async def start(update: Update, context: CallbackContext) -> None:
    options = [
        ...
        ("Discord Nitro ⚡️", 'discord_nitro'),
        ("Чіти на ігри 🎮", 'game_cheats')
    ]
    keyboard = [[InlineKeyboardButton(text, callback_data=data)] for text, data in options]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # Перевіряємо, чи це повідомлення або callback (для коректної обробки в обох випадках)
    if update.message:
...
async def button(update: Update, context: CallbackContext) -> None:
    ...
    if query.data == 'back':
        await start(update, context)  # Повертаємось до головного меню
        return
